  ...
  return {
    ...
    async telegram(text) {
      ...
      try {
        ...
        log.error("[notify] telegram failed:", err.message);
      }
      return false;
    },
    ...
    async email({ to, subject, text, replyTo }) {
      ...
      try {
        ...
        log.error("[notify] resend failed:", err.message);
      }
      return false;
    },
  };
...
export function signupEmail({ key, credits, apiUrl, siteUrl }) {
  return {
...
export function purchaseEmail({ credits, total, apiUrl }) {
  return {
